 ...
 * @param {*} conclusion
 */
async function storeTaskCompleted(taskID, status, finishedAt, completedAt, duration, conclusion) {
  const update = 'UPDATE tasks set status = $2, finishedAt = $3, completedAt = $4, duration = $5, \
   conclusion = $6 \
   where taskID = $1;'
  return await pool.query(update, [taskID, status, finishedAt, completedAt, duration, conclusion])
}
