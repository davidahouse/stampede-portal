 ...
 */
async function recentBuilds(hours, owner, repository) {
  ...
  if (owner != null && repository != null) {
    ...
    return await pool.query(query, ["completed", recent, owner, repository]);
  } else if (owner != null) {
    const query =
      "SELECT * FROM stampede.builds \
      WHERE status = $1 AND \
      ...
      owner = $3 \
      ORDER BY completed_at DESC";
    return await pool.query(query, ["completed", recent, owner]);
  } else {
    const query =
      "SELECT * FROM stampede.builds \
      WHERE status = $1 AND \
 ...
 * @param {string} repository
 */
async function recentTasks(hours, owner, repository) {
  const recent = new Date();
  recent.setHours(recent.getHours() - hours);
  if (owner != null && repository != null) {
    const query =
    ...
    return await pool.query(query, [recent, owner, repository]);
  } else if (owner != null) {
    const query =
      "SELECT * FROM stampede.tasks \
      WHERE finished_at >= $1 AND \
      owner = $2 \
      ORDER BY finished_at DESC";
    return await pool.query(query, [recent, owner]);
  } else {
    const query =
      ...
      ORDER BY finished_at DESC";
    return await pool.query(query, [recent]);
  }
}
