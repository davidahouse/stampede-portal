 ...
 * @param {string} repository
 */
async function recentTasks(timeFilter, taskFilter, repositoryFilter) {
  console.log(timeFilter, taskFilter, repositoryFilter);
  ...
  if (taskFilter === "All") {
    if (repositoryFilter === "All") {
      ...
      return await pool.query(query, [fromDate, toDate]);
    } else {
      const query =
        "SELECT * FROM stampede.builds, stampede.tasks \
      WHERE finished_at >= $1 AND \
      ...
      return await pool.query(query, [
        fromDate,
        toDate,
        repositoryFilter.split("/")[0],
        repositoryFilter.split("/")[1]
      ]);
    }
  } else {
    if (repositoryFilter === "All") {
      ...
      tasks.build_id = builds.build_id \
      ORDER BY finished_at DESC";
      return await pool.query(query, [fromDate, toDate, taskFilter]);
    } else {
      const query =
        "SELECT * FROM stampede.builds, stampede.tasks \
      WHERE finished_at >= $1 AND \
